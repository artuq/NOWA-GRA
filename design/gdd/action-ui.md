...
- **`unlock_threshold` values and future actions behind locked slots** — owned by no current GDD; the v2 prototype has placeholders but they are not design-approved. *Owner: Action System (revision) or a new Milestone/Unlock System GDD. Target: before `/vertical-slice`.*
