...
- **Cosmetic Persona Customization** (Full Vision, Designed 2026-07-23) — **contract resolved**, narrower than originally provisioned: reads `HistoryFlagManager`'s already-shipped `class_path.{path}.best_tier.{N}` milestones (written by this system's `reset_era_state()`, `class_path_system.gd:510`), never the currently-active path directly. Zero new API required on this system.
...
- ~~**BLOCKING — counter-name collision with History Flag System**~~ — **FALSE ALARM, corrected 2026-07-13 (`/propagate-design-change` — verified against actual shipped code, not just the quick-spec's prose)**. The `{path_id}_choices_count` naming this GDD uses (`pato_streamer_choices_count` etc.) is **already** the live scheme: `card_content_database.gd` already has a `path_tag` field on every card, and `class_path_system.gd` already reads `{path_id}_choices_count` correctly — no rename/propagation needed for this part. What the original (incorrect) flag was reaching for actually exists but is different: `HistoryFlagManager.resolve_path_eligibility()` uses a **separate, unrelated, and currently dead-code** `risky_choices_count`/`safe_choices_count` 2-path scheme that nothing in production calls (only its own unit tests) — coexisting confusingly with the real mechanism but not blocking it. **Real remaining gap (not blocking, content not architecture)**: only 6 cards are tagged `pato_streamer` and 2 `guru_celebryta` — zero cards exist yet for `ekspert_niszowy`/`biznesmen_contentu`. That's a narrative-director/writer content task (write or retag cards for the other two paths), not a schema/propagation task. *Owner: whoever picks up card-content authoring for the Vertical Slice full-4-path story. Separately, `resolve_path_eligibility()`'s dead code is a minor cleanup candidate — not urgent, not blocking anything.*
