...
`design/gdd/main-navigation-screen-flow.md` is design-complete (3 `/design-review` rounds, all blocking findings resolved) but `MainNavCoordinator` — the module that owns "which of the two overlay panels (`ClassPathPanel`, `SettingsScreen`) is open" and enforces card-interrupt priority — has zero lines of code. Two real bugs already exist in shipped code that this coordinator must work around: `SettingsScreen._on_close_pressed()` (`settings_screen.gd:47-48`) and `ClassPathPanel`'s equivalent handler (`class_path_panel.gd:141-142`) both set `visible = false` directly on themselves, bypassing any future coordinator entirely. This ADR gives the coordinator a concrete architecture to implement against and formalizes the fix for both Close-button handlers.
...
- Six real entry points must all route through the same resolver (GDD, "Punkty wejścia do resolvera"): PathButton press, SettingsButton press, ClassPathPanel close request, SettingsScreen close request, back-gesture handler, and the `DecisionCardSystem`/`CardScreen` read-only pair.
...
- At most one of the two coordinator-owned panels visible at any time.
...
action_screen.gd (ActionScreen, extends Control)
  ...
  Entry points (6, each a thin wrapper calling _apply_state_change()):
  ...
  _apply_state_change(new_state: CoordinationState) -> void:
    ...
    # _settings_screen.visible as an EFFECT of the new state — never
    # touches CardScreen.visible under any branch
...
# No public setter — coordination_state changes exclusively through the 6 entry
# points above, each funneling into the private _apply_state_change() resolver.
# Guarantee: after ANY of the 6 entry points returns, coordination_state and
# ClassPathPanel.visible/SettingsScreen.visible are mutually consistent —
# no caller ever observes a torn state between them.
```
...
### Negative
- `action_screen.gd` grows in responsibility (TopBar buttons + panel coordination + back-gesture handling + Web JS bridge glue) — still well under the 40-line-per-method standard if each entry point stays a thin wrapper, but the file itself becomes the single busiest script in the UI layer.
...
| main-navigation-screen-flow.md | "Punkty wejścia do resolvera" — 6 entry points, one resolver | `_apply_state_change()` is the sole resolver; all 6 entry points are thin wrappers around it |
...
2. Add `coordination_state` enum + `_apply_state_change()` resolver + the 6 thin entry-point wrappers to `action_screen.gd`.
3. Connect the two new `close_requested` signals in `action_screen.gd::_ready()`.
4. Connect `DecisionCardSystem.card_presented` and `%CardScreen.visibility_changed` (read-only).
5. Add `_notification(NOTIFICATION_WM_GO_BACK_REQUEST)` override (Android).
6. Add Web `JavaScriptBridge`/`pushState` glue, gated on `OS.has_feature("web")`.
7. Set `application/config/quit_on_go_back=false` in `project.godot`.
8. No changes required to `DecisionCardSystem`, `CardScreen`, `ClassPathSystem`, `PrestigeSystem`, `BurnoutSystem`, or `ChallengeSystem` — this ADR is additive on top of all of them.
