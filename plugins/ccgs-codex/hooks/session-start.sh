...
HOOK_SOURCE=$(ccgs_json_get '.source')
...
echo "=== Codex Game Studios (CCGS) — Session Context ==="
...
if [ "$HOOK_SOURCE" = "compact" ] && [ -f "production/session-state/pre-compact.md" ]; then
    echo ""
    echo "=== PRE-COMPACTION CHECKPOINT ==="
    sed -n '1,200p' production/session-state/pre-compact.md
    echo "=== END PRE-COMPACTION CHECKPOINT ==="
fi
