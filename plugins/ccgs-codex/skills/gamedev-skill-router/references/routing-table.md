...
### Godot (`sibling skill folders`, target Godot 4.7) — read after `project.godot` is detected
...
### Unity (`sibling skill folders`, target Unity 6.3 LTS / 6000.3)
...
### Unreal (`sibling skill folders`, target UE 5.8)
...
### Web engines (`sibling skill folders`) — detected from `package.json` + imports
...
### Other engines (`sibling skill folders`)
...
## Disciplines (`sibling skill folders`) — cross-engine concepts, load **with** the engine skill
...
## Genres (`sibling skill folders`) — whole-game templates, **compose** engine + disciplines
...
## Workflows (`sibling skill folders`) — process & shipping, engine-independent
