...
|:-:|--------|--------------------------------|-------------------|----------------|
| 1 | Godot | `project.godot` | `*.tscn`, `*.gd`, `*.gdshader`, `*.tres`, `export_presets.cfg` | `sibling skill folders` |
| 2 | Unreal | `*.uproject` | `Source/**/*.Build.cs`, `Config/Default*.ini`, `*.uasset`, `Content/` | `sibling skill folders` |
| 3 | Unity | `Assets/` **and** `ProjectSettings/ProjectVersion.txt` | `*.unity`, `*.prefab`, `*.asmdef`, `Packages/manifest.json` | `sibling skill folders` |
| 4 | Bevy | `Cargo.toml` with a `bevy` dependency | `*.rs` with `App::new()` / `use bevy::prelude` | `sibling skill folders` |
| 5 | Phaser | `package.json` dep `phaser` | `import Phaser`, `new Phaser.Game` | `phaser-core`, `phaser-arcade-physics` |
| 6 | PixiJS | `package.json` dep `pixi.js` | `import * as PIXI`, `new PIXI.Application` | `sibling skill folders` |
| 7 | three.js | `package.json` dep `three` | `import * as THREE`, `*.gltf` / `*.glb` | `threejs-scene-setup`, `threejs-gltf-loading`, `threejs-materials-lighting` |
| 8 | LÖVE | `conf.lua`, or `main.lua` calling `love.*` | `*.love` | `sibling skill folders` |
| 9 | pygame | `*.py` with `import pygame` | `pygame.init()`, `pygame.display.set_mode` | `sibling skill folders` |
| 10 | Roblox | `*.rbxl` / `*.rbxlx`, or `*.project.json` (Rojo) | `*.luau`, `DataStoreService`, `game:GetService` | `roblox-luau`, `roblox-datastores` |
...
For a new project use the baseline in `version-support.md`. For an existing project,
