...
|:-:|--------|----------------|----------------|
| 1 | Godot | `project.godot` | `sibling skill folders` |
| 2 | Unreal | `*.uproject` | `sibling skill folders` |
| 3 | Unity | `Assets/` **and** `ProjectSettings/ProjectVersion.txt` | `sibling skill folders` |
| 4 | Bevy | `Cargo.toml` with a `bevy` dependency | `sibling skill folders` |
| 5 | Phaser | `package.json` dep `phaser` | `phaser-*` |
| 6 | PixiJS | `package.json` dep `pixi.js` | `sibling skill folders` |
| 7 | three.js | `package.json` dep `three` | `threejs-*` |
| 8 | LÖVE | `conf.lua` / `main.lua` calling `love.*` | `sibling skill folders` |
| 9 | pygame | `*.py` with `import pygame` | `sibling skill folders` |
| 10 | Roblox | `*.rbxl(x)` / `*.project.json` (Rojo) | `roblox-*` |
...
and `references/version-support.md`.
...
- **Godot** (`sibling skill folders`): language `godot-gdscript` / `godot-csharp`; structure
...
- **Unity** (`sibling skill folders`): scripting `unity-csharp-scripting`; input `unity-input-system`;
...
- **Unreal** (`sibling skill folders`): visual scripting `unreal-blueprints`; C++ gameplay
  `unreal-cpp-gameplay`; input `unreal-enhanced-input`; AI `unreal-behavior-trees`; VFX
  `unreal-niagara`; ship `unreal-packaging`.
- **Web** (`sibling skill folders`): `phaser-core`, `phaser-arcade-physics`; `pixijs-rendering`;
  `threejs-scene-setup`, `threejs-gltf-loading`, `threejs-materials-lighting`.
- **Other** (`sibling skill folders`): `bevy-ecs`, `pygame-core`, `love2d-core`, `roblox-luau`,
  `roblox-datastores`.
...
2. **On selection:** read the body of **each chosen** `../<name>/SKILL.md` — and
