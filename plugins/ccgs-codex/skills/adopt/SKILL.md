...
Invoke this workflow as `$adopt` followed by any arguments in the user's prompt.
...
**Audit mode:** `invocation argument 1 from the user text after $adopt` (blank = `full`)
...
If the project appears fresh (no artifacts at all), use `Codex user-input mechanism`:
...
and offer to handle it immediately using `Codex user-input mechanism`. Choose the first
branch that applies:
