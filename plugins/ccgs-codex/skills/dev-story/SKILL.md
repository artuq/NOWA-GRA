...
Invoke this workflow as `$dev-story` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Bash, subagent delegation, user-input`.
...
If they differ, use `Codex user-input mechanism` before proceeding:
...
- **File ownership is delegated** — assign each source/test/evidence file to one implementation owner. The user's `$dev-story` request authorizes those in-scope writes; subagents must not ask for redundant per-file approval. The primary thread coordinates overlaps and decisions.
