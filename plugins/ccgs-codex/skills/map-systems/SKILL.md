...
Invoke this workflow as `$map-systems` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, user-input, TodoWrite, subagent delegation`.
...
- Use `Codex user-input mechanism` to ask:
...
Then use `Codex user-input mechanism` to capture feedback:
...
Use `Codex user-input mechanism` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` through Codex subagent delegation using gate TD-SYSTEM-BOUNDARY (`plugins/ccgs-codex/references/director-gates.md`) before proceeding to priority assignment.**
...
Use `Codex user-input mechanism` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` through Codex subagent delegation using gate PR-SCOPE (`plugins/ccgs-codex/references/director-gates.md`) before writing the index.**
...
Using the template at `plugins/ccgs-codex/assets/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` through Codex subagent delegation using gate CD-SYSTEMS (`plugins/ccgs-codex/references/director-gates.md`).**
...
Use `Codex user-input mechanism` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `$design-system` completes, use `Codex user-input mechanism`:
...
After the systems index is created (or after designing some systems), present next actions using `Codex user-input mechanism`:
...
2. **Codex user-input mechanism** at every decision point (Explain -> Capture pattern):
