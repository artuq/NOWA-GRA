...
Invoke this workflow as `$create-control-manifest` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, subagent delegation`.
...
Spawn `technical-director` through Codex subagent delegation using gate **TD-MANIFEST** (`plugins/ccgs-codex/references/director-gates.md`).
...
- **CONCERNS** → surface via `Codex user-input mechanism` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
