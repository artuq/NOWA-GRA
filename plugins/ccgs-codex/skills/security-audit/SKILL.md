...
Invoke this workflow as `$security-audit` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash, Write, subagent delegation`.
Recommended role: `security-engineer` from `.codex/agents/security-engineer.toml`. Keep the primary thread responsible for user-facing choices.
