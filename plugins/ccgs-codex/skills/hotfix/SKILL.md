...
Invoke this workflow as `$hotfix` followed by any arguments in the user's prompt.
...
Confirm with `Codex user-input mechanism`:
...
If the check passes, use `Codex user-input mechanism` before creating the branch:
...
Use Codex subagent delegation to request sign-off in parallel:
