...
Invoke this workflow as `$team-polish` followed by any arguments in the user's prompt.
...
> Usage: `$team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use `Codex user-input mechanism` here; output the guidance directly.
