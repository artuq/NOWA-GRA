...
Invoke this workflow as `$review-all-gdds` followed by any arguments in the user's prompt.
...
**Focus:** `invocation argument 1 from the user text after $review-all-gdds` (blank = `full`)
...
Use `Codex user-input mechanism` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `Codex user-input mechanism`:
...
After all file writes are complete, use `Codex user-input mechanism` for a closing widget.
