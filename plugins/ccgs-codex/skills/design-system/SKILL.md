...
Invoke this workflow as `$design-system` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, subagent delegation, user-input, TodoWrite`.
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `Codex user-input mechanism`:
...
Use the template structure from `plugins/ccgs-codex/assets/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `Codex user-input mechanism` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `Codex user-input mechanism` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `Codex user-input mechanism`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `Codex user-input mechanism` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `Codex user-input mechanism`:
...
- Surface any disagreements between agents to the user via `Codex user-input mechanism`
...
- Present the specialists' proposals to the user for review via `Codex user-input mechanism`
...
Before finalizing the GDD, spawn `creative-director` through Codex subagent delegation using gate **CD-GDD-ALIGN** (`plugins/ccgs-codex/references/director-gates.md`).
...
**When delegating through Codex subagent delegation tool**:
...
- The main session presents the agent's output to the user via `Codex user-input mechanism`
- The user decides; the main session writes to file
