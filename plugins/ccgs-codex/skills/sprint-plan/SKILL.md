...
Invoke this workflow as `$sprint-plan` followed by any arguments in the user's prompt.
...
Legacy dynamic context becomes an explicit read-only preflight when relevant: `!ls production/sprints/ 2>/dev/null`
...
- If the file doesn't exist and this is a `new` sprint: use `Codex user-input mechanism`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `Codex user-input mechanism` to gather changes.
...
Before finalising the sprint plan, spawn `producer` through Codex subagent delegation using gate **PR-SPRINT** (`plugins/ccgs-codex/references/director-gates.md`).
...
If CONCERNS, use `Codex user-input mechanism`:
