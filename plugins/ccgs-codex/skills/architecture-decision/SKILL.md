...
Invoke this workflow as `$architecture-decision` followed by any arguments in the user's prompt.
...
a **confirm/adjust** prompt using `Codex user-input mechanism` — not open-ended questions.
...
decisions remain, present each one as a separate `Codex user-input mechanism` with the proposed
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` through Codex subagent delegation using gate **TD-ADR** (`plugins/ccgs-codex/references/director-gates.md`):
...
5. **Write approval** — Use `Codex user-input mechanism`:
...
Ask using `Codex user-input mechanism`:
...
After the ADR is written (and registry optionally updated), close with `Codex user-input mechanism`.
