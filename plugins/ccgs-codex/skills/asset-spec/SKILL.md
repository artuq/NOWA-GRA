...
Invoke this workflow as `$asset-spec` followed by any arguments in the user's prompt.
...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `Codex user-input mechanism`:
...
Present the full proposed inventory to the user in conversation. Then use `Codex user-input mechanism`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `Codex user-input mechanism`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `Codex user-input mechanism`:
...
Present the full identified list to the user. Use `Codex user-input mechanism`:
...
Spawn specialist agents based on review mode. **Issue all subagent tasks simultaneously — do not wait for one before starting the next.**
...
After presenting all specs, use `Codex user-input mechanism`:
