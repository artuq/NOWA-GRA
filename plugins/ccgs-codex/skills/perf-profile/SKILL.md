...
Invoke this workflow as `$perf-profile` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash`.
Recommended role: `performance-analyst` from `.codex/agents/performance-analyst.toml`. Keep the primary thread responsible for user-facing choices.
