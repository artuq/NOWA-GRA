...
Invoke this workflow as `$content-audit` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write`.
Recommended role: `producer` from `.codex/agents/producer.toml`. Keep the primary thread responsible for user-facing choices.
