...
Invoke this workflow as `$team-qa` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, subagent delegation, user-input`.
Recommended role: `qa-lead` from `.codex/agents/qa-lead.toml`. Keep the primary thread responsible for user-facing choices.

---

When this skill is invoked, orchestrate the QA team through a structured testing cycle.

**Decision Points:** At each phase transition, use `Codex user-input mechanism` to present
...
Spawn `qa-lead` through Codex subagent delegation to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `Codex user-input mechanism`:
...
Spawn `qa-tester` through Codex subagent delegation for each story (run in parallel where possible), providing:
- The story file path
...
Use `Codex user-input mechanism` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `Codex user-input mechanism` for each:
...
After each FAIL result: use `Codex user-input mechanism` to collect the failure description, then spawn `qa-tester` through Codex subagent delegation to write a formal bug report in `production/qa/bugs/`.
