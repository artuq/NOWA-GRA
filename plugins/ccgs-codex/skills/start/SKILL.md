...
Invoke this workflow as `$start` followed by any arguments in the user's prompt.
...
This is the first thing the user sees. Use `Codex user-input mechanism` with these exact options so the user can click rather than type:

- **Prompt**: "Welcome to Codex Game Studios (CCGS)! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not Codex user-input mechanism (it's an open response).
2. Acknowledge the concept, then use `Codex user-input mechanism` to offer two paths:
...
**If it does not exist**: Use `Codex user-input mechanism`:
...
After presenting the recommended path, use `Codex user-input mechanism` to ask the user which step they'd like to take first. Never auto-run the next skill.
