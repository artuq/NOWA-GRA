...
Invoke this workflow as `$qa-plan` followed by any arguments in the user's prompt.
...
**Argument:** `the invocation arguments after $qa-plan` (blank = ask user via Codex user-input mechanism)
...
- **No argument** — use `Codex user-input mechanism`:
...
then ask two questions together using `Codex user-input mechanism`:
...
- Use `Codex user-input mechanism` for scope selection when no argument is provided.
