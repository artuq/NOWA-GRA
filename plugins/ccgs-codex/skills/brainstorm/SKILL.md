...
Invoke this workflow as `$brainstorm` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, WebSearch, subagent delegation, user-input`.
...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   See `plugins/ccgs-codex/references/director-gates.md` for the full check pattern.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `Codex user-input mechanism`** at key decision points throughout brainstorming:
   ...
   `Codex user-input mechanism` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an Codex user-input mechanism with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `Codex user-input mechanism` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `Codex user-input mechanism` with these exact tab labels:
...
Present all three. Then use `Codex user-input mechanism` to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
Codex user-input mechanism(
...
Ask these as `Codex user-input mechanism` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `Codex user-input mechanism`:
...
If the user selects B, C, or D, make the revision, then use `Codex user-input mechanism` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` through Codex subagent delegation in parallel before moving to Phase 5. Issue both subagent tasks simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`plugins/ccgs-codex/references/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`plugins/ccgs-codex/references/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `Codex user-input mechanism`:
...
- **Target platform**: Use `Codex user-input mechanism` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `Codex user-input mechanism` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` through Codex subagent delegation using gate TD-FEASIBILITY (`plugins/ccgs-codex/references/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` through Codex subagent delegation using gate PR-SCOPE (`plugins/ccgs-codex/references/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `plugins/ccgs-codex/assets/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `Codex user-input mechanism` for write approval:
...
If [B]: ask which section to revise using `Codex user-input mechanism` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `Codex user-input mechanism` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `plugins/ccgs-codex/assets/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
