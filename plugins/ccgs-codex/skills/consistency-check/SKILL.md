...
Invoke this workflow as `$consistency-check` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, Bash, user-input`.
...
Then close with an `Codex user-input mechanism` widget:
