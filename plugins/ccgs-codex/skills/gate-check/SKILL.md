...
Invoke this workflow as `$gate-check` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash, Write, subagent delegation, user-input`.
...
**Target phase:** `invocation argument 1 from the user text after $gate-check` (blank = auto-detect current stage, then validate next transition)
...
- **No argument**: Auto-detect current stage using the same heuristics as
  ...
  Use `Codex user-input mechanism`:
...
Before generating the final verdict, delegate all four director reviews using the protocol in `plugins/ccgs-codex/references/director-gates.md`. Start as many independent reviews as the active slot limit permits, collect them, then run the remaining reviews as the next batch. Never exceed the concurrency limit.

**Director review batch:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`plugins/ccgs-codex/references/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`plugins/ccgs-codex/references/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`plugins/ccgs-codex/references/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`plugins/ccgs-codex/references/director-gates.md`)
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `Codex user-input mechanism`.
...
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `plugins/ccgs-codex/assets/templates/difficulty-curve.md` — or use `$quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `plugins/ccgs-codex/assets/templates/player-journey.md` — or author it collaboratively using `$ux-design` Phase 2b.
