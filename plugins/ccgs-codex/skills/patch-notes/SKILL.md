...
Invoke this workflow as `$patch-notes` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Bash`.
Recommended role: `community-manager` from `.codex/agents/community-manager.toml`. Keep the primary thread responsible for user-facing choices.
...
1. Glob for `docs/patch-notes-template.md` and `plugins/ccgs-codex/assets/templates/patch-notes-template.md`.
