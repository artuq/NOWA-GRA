...
Invoke this workflow as `$retrospective` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Bash, user-input`.
...
If a matching file is found, use `Codex user-input mechanism`:
...
Then use `Codex user-input mechanism` to present two options:
