...
Invoke this workflow as `$changelog` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash, Write`.
Legacy dynamic context becomes an explicit read-only preflight when relevant: `!git log --oneline -30 2>/dev/null !git tag --list --sort=-v:refname 2>/dev/null | head -5`
