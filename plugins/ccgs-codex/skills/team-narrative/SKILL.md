...
Invoke this workflow as `$team-narrative` followed by any arguments in the user's prompt.
...
> Usage: `$team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `Codex user-input mechanism` here; output the guidance directly.
...
Delegate in parallel — issue all three subagent tasks simultaneously before waiting for any result:
...
sub-agents spawned through Codex subagent delegation. Each sub-agent enforces the "May I write to [path]?"
