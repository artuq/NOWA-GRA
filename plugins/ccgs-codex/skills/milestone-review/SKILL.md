...
Invoke this workflow as `$milestone-review` followed by any arguments in the user's prompt.
...
Before generating the Go/No-Go recommendation, spawn `producer` through Codex subagent delegation using gate **PR-MILESTONE** (`plugins/ccgs-codex/references/director-gates.md`).
...
If OFF TRACK, use `Codex user-input mechanism` before generating the recommendation:
...
If AT RISK, use `Codex user-input mechanism`:
