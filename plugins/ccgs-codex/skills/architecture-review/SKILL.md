...
Invoke this workflow as `$architecture-review` followed by any arguments in the user's prompt.
...
If [B]: display the complete proposed systems-index section, then re-ask with `Codex user-input mechanism`.
...
Use `Codex user-input mechanism` for the write approval:
...
For `rtm` mode, use `Codex user-input mechanism`:
...
Then close with `Codex user-input mechanism` tailored to the pre-gate checklist state:
...
3. **Offer options** via Codex user-input mechanism with three choices:
