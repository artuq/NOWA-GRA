...
Invoke this workflow as `$propagate-design-change` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Bash, subagent delegation`.
...
Spawn `technical-director` through Codex subagent delegation using gate **TD-CHANGE-IMPACT** (`plugins/ccgs-codex/references/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `Codex user-input mechanism` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
