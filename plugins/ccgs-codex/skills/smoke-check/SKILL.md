...
Invoke this workflow as `$smoke-check` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash, Write, user-input`.
...
Use `Codex user-input mechanism` to batch-verify. Keep to at most 3 calls.
...
- Use `Codex user-input mechanism` for all manual smoke check verification.
