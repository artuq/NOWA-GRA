...
Invoke this workflow as `$create-architecture` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Bash, user-input, subagent delegation`.
Recommended role: `technical-director` from `.codex/agents/technical-director.toml`. Keep the primary thread responsible for user-facing choices.
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `Codex user-input mechanism`:
...
Apply gate **TD-ARCHITECTURE** (`plugins/ccgs-codex/references/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` through Codex subagent delegation using gate LP-FEASIBILITY (`plugins/ccgs-codex/references/director-gates.md`):**
...
Use `Codex user-input mechanism` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `Codex user-input mechanism`:
...
5. **Use `Codex user-input mechanism` for write approvals** — plain text "May I?" is not
