...
Invoke this workflow as `$setup-engine` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, WebSearch, WebFetch, subagent delegation, user-input`.
...
**Question 1 — Prior experience** (ask this first, always, via `Codex user-input mechanism`):
...
**Question 2 — Target platform** (ask this second, always, via `Codex user-input mechanism` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `Codex user-input mechanism` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `Codex user-input mechanism` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use `Codex user-input mechanism`:
