...
Invoke this workflow as `$help` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep`.
Legacy dynamic context becomes an explicit read-only preflight when relevant: `!echo "=== Live Project State ===" && echo "Stage: $(cat production/stage.txt 2>/dev/null | tr -d '[:space:]' || echo 'not set')" && echo "Latest sprint: $(ls -t production/sprints/*.md 2>/dev/null | head -1 || echo 'none')" && echo "Session state: $(head -5 production/session-state/active.md 2>/dev/null || echo 'none')"`
...
Read `plugins/ccgs-codex/references/workflow-catalog.yaml`. This is the authoritative list of all
...
After reading the catalog, Glob `plugins/ccgs-codex/skills/*/SKILL.md` to get the full list
