...
Invoke this workflow as `$design-review` followed by any arguments in the user's prompt.
...
**CRITICAL: Codex subagent delegation in this skill starts a SUBAGENT — a separate independent Codex session
...
actual subagent tasks. A simulated review is not a specialist review.**

Issue all subagent tasks simultaneously. Do NOT spawn one at a time.
...
Use `Codex user-input mechanism` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `Codex user-input mechanism` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `Codex user-input mechanism` for a **post-revision closing widget**:

- Prompt: "Revisions complete — [N] blockers resolved. What next?"
- Note current context usage: if context is above ~50%, add: "(Recommended: start a fresh thread before re-review — this session has used X% context. A full re-review runs 5 agents and needs clean context.)"
- Options:
  - `[A] Re-review in a new session — run $design-review [doc-path] after start a fresh thread`
...
When the verdict is APPROVED, use a single `Codex user-input mechanism` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `Codex user-input mechanism`:
...
Use a third `Codex user-input mechanism`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `Codex user-input mechanism`:
