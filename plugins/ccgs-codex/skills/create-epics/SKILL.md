...
Invoke this workflow as `$create-epics` followed by any arguments in the user's prompt.
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` through Codex subagent delegation using gate **PR-EPIC** (`plugins/ccgs-codex/references/director-gates.md`).
