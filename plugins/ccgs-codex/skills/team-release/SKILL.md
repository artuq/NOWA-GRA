...
Invoke this workflow as `$team-release` followed by any arguments in the user's prompt.
...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `Codex user-input mechanism`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `Codex user-input mechanism` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
