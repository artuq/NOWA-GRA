...
Invoke this workflow as `$team-audio` followed by any arguments in the user's prompt.
...
> Usage: `$team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `Codex user-input mechanism` here; output the guidance directly.
