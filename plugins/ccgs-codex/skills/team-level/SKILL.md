...
Invoke this workflow as `$team-level` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, Bash, subagent delegation, user-input, TodoWrite`.

---

When this skill is invoked:

**Decision Points:** At each step transition, use `Codex user-input mechanism` to present
...
Use Codex subagent delegation to spawn each team member as a subagent:
...
Spawn all three agents simultaneously — issue all three subagent tasks before waiting for any result.
...
**Gate**: Use `Codex user-input mechanism` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `Codex user-input mechanism` with options:
...
**Gate**: Use `Codex user-input mechanism` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `Codex user-input mechanism` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `Codex user-input mechanism` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
to sub-agents spawned through Codex subagent delegation. Each sub-agent enforces the "May I write to [path]?"
...
3. **Offer options** via Codex user-input mechanism with choices:
