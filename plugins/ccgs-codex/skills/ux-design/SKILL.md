...
Invoke this workflow as `$ux-design` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, user-input, subagent delegation`.
Recommended role: `ux-designer` from `.codex/agents/ux-designer.toml`. Keep the primary thread responsible for user-facing choices.
...
**If no argument is provided**, do not fail — ask instead. Use `Codex user-input mechanism`:
...
> "Player journey map not yet created. Template available at `plugins/ccgs-codex/assets/templates/player-journey.md`. Run `$ux-design` Phase 2b or create it manually to establish player context for this screen."
...
2. **Questions**: Ask what is needed to draft this section. Use `Codex user-input mechanism`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `Codex user-input mechanism` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `Codex user-input mechanism`:
...
7. **Write**: Use `Codex user-input mechanism`: "May I write the [section name] section to `[filepath]`?"
...
- Use `Codex user-input mechanism` to capture the choice:
...
- Use `Codex user-input mechanism`: "Want an ASCII wireframe as part of this spec?"
...
Use `Codex user-input mechanism` to confirm:
...
Use `Codex user-input mechanism` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `Codex user-input mechanism`:
...
Then use `Codex user-input mechanism`:
...
When delegating to another agent via Codex subagent delegation:
