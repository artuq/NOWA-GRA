...
Invoke this workflow as `$create-stories` followed by any arguments in the user's prompt.
...
Recommended role: `lead-programmer` from `.codex/agents/lead-programmer.toml`. Keep the primary thread responsible for user-facing choices.
...
in this skill — apply the check pattern from `plugins/ccgs-codex/references/director-gates.md`
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` through Codex subagent delegation using gate **QL-STORY-READY** (`plugins/ccgs-codex/references/director-gates.md`).
...
Use `Codex user-input mechanism` to close with context-aware next steps:
