...
Invoke this workflow as `$story-done` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Bash, Write, Edit, user-input, subagent delegation`.
...
3. If multiple in-progress stories are found, use `Codex user-input mechanism`:
...
### Manual verification with confirmation (use `Codex user-input mechanism`)
...
Batch up to 4 manual verification questions into a single `Codex user-input mechanism` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via `Codex user-input mechanism`
...
After completing the deviation checks in Phase 4, spawn `qa-lead` through Codex subagent delegation using gate **QL-TEST-COVERAGE** (`plugins/ccgs-codex/references/director-gates.md`).
...
- `lean` → use `Codex user-input mechanism` before proceeding:
...
Spawn `lead-programmer` through Codex subagent delegation using gate **LP-CODE-REVIEW** (`plugins/ccgs-codex/references/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `Codex user-input mechanism`:
...
Use `Codex user-input mechanism` before writing anything:
...
- Use `Codex user-input mechanism` for the code review prompt and for batching manual
