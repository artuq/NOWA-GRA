...
Invoke this workflow as `$prototype` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, Write, Edit, Bash, subagent delegation, user-input`.
Recommended role: `prototyper` from `.codex/agents/prototyper.toml`. Keep the primary thread responsible for user-facing choices.
This workflow requested isolated worktree execution upstream. Use an isolated worktree only when the parent environment explicitly provides one; otherwise keep edits narrowly scoped in the current workspace.
...
Otherwise, use `Codex user-input mechanism` to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then use `Codex user-input mechanism` with your
...
Read `plugins/ccgs-codex/assets/templates/prototype-report.md` to get the report structure.
...
2. **Choose path** — same Codex user-input mechanism widget as Phase 3 (HTML / Engine / Paper).
