...
Invoke this workflow as `$story-readiness` followed by any arguments in the user's prompt.
...
Legacy tool profile (guidance only): `Read, Glob, Grep, user-input, subagent delegation`.
...
See `plugins/ccgs-codex/references/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `invocation argument 1 from the user text after $story-readiness` (blank = ask user via Codex user-input mechanism)
...
If no argument is given, use `Codex user-input mechanism`:
...
Spawn `qa-lead` through Codex subagent delegation using gate **QL-STORY-READY** (`plugins/ccgs-codex/references/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `Codex user-input mechanism`:
