...
- **Touch Support**: Full
- **Platform Notes**: Wszystkie akcje podstawowe muszą działać przez duże, dotykowe obszary realizowane standardowym węzłem `Button` (otrzymuje syntezowane zdarzenia dotykowe project-wide), nie `TouchScreenButton` — to przestarzały węzeł `Node2D` niezintegrowany z układem/themingiem `Control` (per ADR-0007, engine-specialist 2026-06-24). Brak hover-only interakcji. **CrazyGames SDK v3 zintegrowany i utwardzony (2026-08-25)**: bootstrap w `export_presets.cfg` `html/head_include` (`window.kocSDK` — init, idempotentne loadingStart/Stop, przejścia gameplayStart/Stop, happytime i Data Module), wywołania z GDScript przez `JavaScriptBridge` gated na `OS.has_feature("web")`. Ekrany boot/menu/panel nie są raportowane jako gameplay, a boot czeka na account-aware zapis przed restore/offline simulation; kod jest no-op poza Web i nie wchodzi na ścieżkę Android. Pierwsze zgłoszenie: **Basic Launch**, bez reklam; reklamy pozostają zakresem późniejszego Full Launch.
