...
After any disruption (compaction, crash, `start a fresh thread`), read the state file first.
...
- **Use `start a fresh thread`** between unrelated tasks, or after 2+ failed correction attempts
