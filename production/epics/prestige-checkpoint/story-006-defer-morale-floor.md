...
- [ ] GIVEN Choice B (Defer) is taken with Morale below `BURNOUT_DEFER_MORALE_COST`, WHEN Defer resolves, THEN Morale clamps to `0` (never negative), no path read occurs (`get_active_path()`/`get_tier()` are not called), all four `META_BONUS_total[type]` remain unchanged, AND `burnout_deferred_era_N` (matching the current era) is set and preserved as a meta-persistent flag
